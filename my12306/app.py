...
class Ticket(object):
    ...
    def initialize(self):
        ...
        if dic == "":
            ...
            RAIL_EXPIRATION = "1576787322748"
            RAIL_DEVICEID = "mSy4uz_1bgsw8mRL6ZKHwiFd8mjDvHg9fYID37gyKM52tNlFp_6hUjTJ0lnjWOh6d-NkmUQPa9jhJgYqnP2Dkd3XmJNxpdjXbceefYl0nmsYSmVblSA0VI2YtuigRHrJw9c7lO8EoeEF-ufBQ_dqlmaJ4QA21QhV"
        else:
            print("网络获取指纹成功!")
    ...
    def get_img_code(self):
        try:
            response = SessionUtil.session.get(url=UrlUtils.Down_mg_url, headers=Headers.BaseHead)
            req = response.request
            print(req.headers)
            print(response)
            #response = requests.get(url=UrlUtils.Down_mg_url, headers=Headers.BaseHead)
            img_str = re.findall('"image":"(.*?)",', response)
            #img_str = response.json()['image']
            print(img_str)
            #imgdata=base64.b64decode(img_str)
            #with open(os.path.dirname(__file__)+'/code.jpg', 'wb') as f:
            #    f.write(imgdata)
        except Exception:
            print("下载图片错误! 等待重试~~")
            time.sleep(1)
    ...
    def check_img(self):
        try:
            Image.open(os.path.dirname(__file__)+'/code.jpg').show()
        except Exception:
            time.sleep(2)
            Image.open(os.path.dirname(__file__)+'/code.jpg').show()
        print("+---1-------+----------+----------+----------+")
    ...
    def login(self, answer_code):
        ...
        password = ''
        try:
            with open(os.path.dirname(__file__)+"/user.conf", "r", encoding="utf-8") as  f:
    ...
    def saveCookie(self):
        SessionUtil.save_cookies(os.path.dirname(__file__)+"/12306cookies.txt")
...
if __name__ == '__main__':
    ...
    result = ""
    try:
         with open(os.path.dirname(__file__)+"/12306cookies.txt", "r", encoding="utf-8") as fr:
            result = fr.read()
    except Exception:
         pass
    if result != "":
        SessionUtil.load_cookies(os.path.dirname(__file__)+"/12306cookies.txt")
        t.initialize()
        t.conf()
        t.uamtkstatic()
        t.userLogin()
