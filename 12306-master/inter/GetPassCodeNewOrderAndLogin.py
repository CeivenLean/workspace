...
def getPassCodeNewOrderAndLogin(session, imgType):
    ...
    if imgType == "login":
        ...
        codeImgUrl["req_url"] = codeImgUrl["req_url"].format(int(time.time() * 1000))
    else:
    ...
    try:
        if isinstance(result, dict):
            print(u"下载验证码失败, 请手动检查是否ip被封，或者重试，请求地址：https://kyfw.12306.cn{0}".format(codeImgUrl.get("req_url")))
            return False
        else:
            print(u"下载验证码成功")
            try:
